    ...
    <div className="page-container max-w-5xl">
            ...
            {editing ? (
              <div className="space-y-4">
                <input
                  ...
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="BOM名称"
                />
                <textarea
              ...
              <>
                ...
                {bom.description && (
                  <p className="text-gray-500 ml-15">{bom.description}</p>
                )}
              </>
            )}
          </div>
            ...
            {editing ? (
              <>
                <button
                  ...
                  className="px-5 py-3 border border-gray-200 text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-50 transition-all"
                >
                  取消
                </button>
                <button
              ...
              <>
                <button
                  ...
                  className="px-5 py-3 border border-gray-200 text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-50 transition-all flex items-center gap-2"
                >
      ...
      <div className="bg-white rounded-2xl border border-gray-200/80 overflow-hidden section shadow-sm">
        <div className="px-8 py-5 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-gray-900">器件列表</h2>
          </div>
          <span className="text-sm text-gray-500">{displayItems.length} 个器件</span>
        </div>

        {displayItems.length === 0 ? (
          <div className="p-12 text-center">
            <Package className="w-12 h-12 text-gray-300 mx-auto mb-3" />
            <p className="text-gray-500">暂无器件</p>
            <p className="text-gray-400 text-sm mt-1">点击&quot;添加器件&quot;按钮添加</p>
          </div>
        ) : (
              ...
              return (
                <div key={item.partId} className="px-8 py-5 flex items-center gap-6">
                  <div className="flex-1 min-w-0">
                    <Link href={`/parts/${item.partId}`} className="text-sm font-semibold text-gray-900 hover:text-blue-600">
                      {item.name}
                    </Link>
                    ...
                    {editing ? (
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="w-16 px-2 py-2 text-center border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                        <button
                          onClick={() => updateItemQuantity(item.partId, item.quantity + 1)}
                          className="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-lg hover:bg-gray-200"
                      ...
                      <div className="text-right">
                        <p className="text-sm font-bold text-gray-900">需要: {item.quantity} {item.unit}</p>
                        <p className={`text-xs ${stockStatus === "sufficient" ? "text-emerald-600" : "text-red-600"}`}>
      ...
      {showAddPart && (
        <div className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-gray-200/80 max-h-[80vh] overflow-hidden flex flex-col">
            <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-900">添加器件到BOM</h2>
              <button
              ...
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
