...
import { Plus, Package, Trash2, MapPin, Star, Loader2, X, Warehouse } from "lucide-react";
    ...
    <div className="page-container">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 section">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-[var(--card-foreground)] tracking-tight">仓库管理</h1>
            <p className="text-gray-500 dark:text-[var(--foreground-subtle)] mt-1">管理多个仓库库存</p>
          </div>
        </div>
        <button
      ...
      {warehouses.length === 0 ? (
        <div className="bg-white dark:bg-[var(--card)] rounded-2xl border border-gray-200/80 dark:border-[var(--card-border)] p-16 text-center section">
          <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gray-100 dark:bg-[var(--background-muted)] flex items-center justify-center">
            <Warehouse className="w-8 h-8 text-gray-400 dark:text-[var(--foreground-subtle)]" />
          </div>
          <p className="text-gray-500 dark:text-[var(--foreground-subtle)] font-medium">暂无仓库</p>
          <p className="text-gray-400 dark:text-[var(--foreground-subtle)] text-sm mt-1">点击上方按钮创建第一个仓库</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-1">
                  {wh.isDefault ? (
                    <span className="inline-flex items-center gap-1 px-2 py-1 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 rounded-lg text-xs font-medium">
              ...
              <h3 className="text-lg font-semibold text-gray-900 dark:text-[var(--card-foreground)] mb-2">{wh.name}</h3>
              {wh.location && (
                <div className="flex items-center gap-1 text-sm text-gray-500 dark:text-[var(--foreground-subtle)] mb-2">
                  <MapPin className="w-4 h-4" />
                  {wh.location}
                </div>
              )}
              {wh.description && (
                <p className="text-sm text-gray-500 dark:text-[var(--foreground-subtle)] mb-4 line-clamp-2">{wh.description}</p>
              )}
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-[var(--card-border)]">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm text-gray-600 dark:text-[var(--foreground-muted)]">{wh.partCount} 种器件</span>
                  </div>
                  <div className="text-sm text-gray-600 dark:text-[var(--foreground-muted)]">
                    {wh.totalStock || 0} 件
                  </div>
                </div>
              </div>
    ...
    <div className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white dark:bg-[var(--card)] rounded-2xl w-full max-w-lg shadow-2xl border border-gray-200/80 dark:border-[var(--card-border)]">
        <div className="px-8 py-6 border-b border-gray-100 dark:border-[var(--card-border)] flex items-center justify-between rounded-t-2xl">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-[var(--card-foreground)]">新建仓库</h2>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5">
          <div>
            ...
            <input
              ...
              className="w-full px-5 py-4 bg-gray-50 dark:bg-[var(--background-subtle)] border border-gray-200 dark:border-[var(--card-border)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white dark:focus:bg-[var(--card)] transition-all"
              placeholder="如：主仓库、A区仓库"
            />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 dark:text-[var(--foreground-muted)] mb-2">位置</label>
            <input
              value={location}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-5 py-4 bg-gray-50 dark:bg-[var(--background-subtle)] border border-gray-200 dark:border-[var(--card-border)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white dark:focus:bg-[var(--card)] transition-all resize-none"
              rows={3}
              placeholder="仓库描述（可选）"
            />
          </div>
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="isDefault" className="text-sm text-gray-700 dark:text-[var(--foreground-muted)]">
              设为默认仓库
            </label>
          </div>
