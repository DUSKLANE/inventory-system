    ...
    <div className="page-container max-w-3xl">
      ...
      <div className="section">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-[var(--card-foreground)] tracking-tight">入库</h1>
          ...
          <div className="flex-1 flex gap-2 sm:gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-9 sm:pl-12 pr-3 sm:pr-4 py-3 sm:py-4 bg-gray-50 dark:bg-[var(--background-subtle)] border border-gray-200 dark:border-[var(--card-border)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent focus:bg-white dark:focus:bg-[var(--card)] transition-all duration-200"
                placeholder="输入器件编码"
              />
            </div>
            <button
      ...
      {part && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-[var(--card)] rounded-2xl border border-gray-200/80 dark:border-[var(--card-border)] p-4 sm:p-8 shadow-sm mt-4 sm:mt-6">
          <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center">
              <FileText className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 dark:text-blue-400" />
            </div>
            <h2 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-[var(--card-foreground)]">入库信息</h2>
          </div>
          ...
          {success && (
            <div className="mb-4 sm:mb-6 p-4 sm:p-5 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 rounded-lg sm:rounded-xl">
              <div className="flex items-center gap-3 mb-3 sm:mb-4">
                ...
                <p className="text-xs sm:text-sm text-emerald-700 dark:text-emerald-400 font-semibold">入库成功！库存已更新为 {part.stock?.quantity ?? 0} {part.unit}</p>
              </div>
          ...
          <div className="flex flex-col gap-5 sm:gap-8">
            ...
            <div>
              <label className="text-xs sm:text-sm font-semibold text-gray-700 dark:text-[var(--foreground-muted)] mb-1.5 sm:mb-3 flex items-center gap-1.5 sm:gap-2">
                <User className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-gray-500 dark:text-[var(--foreground-subtle)]" /> 操作人
              </label>
              <input
                type="text"
                value={operator}
                onChange={(e) => setOperator(e.target.value)}
                className="w-full px-3 py-2 sm:px-5 sm:py-4 bg-gray-50 dark:bg-[var(--background-subtle)] border border-gray-200 dark:border-[var(--card-border)] rounded-lg sm:rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent focus:bg-white dark:focus:bg-[var(--card)] transition-all duration-200"
                placeholder="可选"
              />
            </div>
            <div>
              <label className="text-xs sm:text-sm font-semibold text-gray-700 dark:text-[var(--foreground-muted)] mb-1.5 sm:mb-3 flex items-center gap-1.5 sm:gap-2">
                <FileText className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-gray-500 dark:text-[var(--foreground-subtle)]" /> 入库原因
              </label>
              <input
                type="text"
                value={reason}
